            ...
            if sub_str in strs:
                ...
                for key in ["train","val"]:
                    ...
                    cfmat = get_confusion_matrix(f[key+"/labels"][:],
                                                 f[key+"/td_outputs"][:],#f[key+"/outputs"][:],
                                                 np.float32)
                ...
                if has_image:
                    ...
                    for key in ["val"]:
                        ...
                        for i in range(img.shape[0]):
                            cmap_num=0
                            for cmap in ["nipy_spectral_r","ocean","spring","inferno"]:
                                cmap_num+=1
                                fig, ax = plt.subplots( nrows=1, ncols=1 )
                                im = ax.imshow(img[i,:,:,0],cmap=cmap,vmin=0,vmax=1.0)
                                fig.colorbar(im, ax=ax)
                                fig.savefig(img_loc+os.sep+key+"_"+str(i)+"_"+str(sr[i])+"_cm"+str(cmap_num)+".png",dpi=200)
