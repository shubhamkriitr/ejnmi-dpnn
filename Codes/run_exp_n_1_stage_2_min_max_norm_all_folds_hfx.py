...
for fd in range(1,max_fold+1):
    ...
    print(fd, path)
    assert(len(path)==1)
    stage_1_models[fd]=path[0][:-5]

print(stage_1_models)
wait = input("wait: Verify the Stage-I models to be used for every fold.")
...
for lr in lrs:
    set_num+=1
    set_id =ut.append_time_string( "_SET_STAGEII_{}_lr1e6_{}_".format(set_num,int(lr*1e6) ))+"_"
    ...
    for fold in range(1,max_fold+1):
        ...
        data only from Dataset-2 also. TF DATA is also normalized to 0..1\
        and SSIM + L2 loss is being used."
        log_list.append({"Note":note})
        log_list.append({"WTS loaded from:":stage_1_models[fold]})
        ...
        model_name = "EXP_N_1_STAGEII_"+set_id+"fold_"+str(fold)+"_"
        ...
        with tf.Session(graph = comp_model.graph,
            ...
            print("Variables initialized --Loading Weights from:",stage_1_models[fold])
            comp_model.saver.restore(sess,stage_1_models[fold])
