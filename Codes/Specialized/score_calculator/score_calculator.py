...
# -*- coding: utf-8 -*-
"""
Created on Sat Jan 20 21:08:54 2018
...
print(type(np.int16))
import projection_net_avg_pooling_for_eval as net_module# TODO_ remove
net = net_module.ProjectionNet
import imp
...
def get_data_generators (fold_number):
    ...
    X , Y = data.get_parkinson_data(DSET_LOC,ranges=r)
    X=X[:,:,:,:,:]
    ...
    Y = Y[:,:,0]
    X_val, Y_val = data.get_parkinson_data(DSET_LOC, ranges=s)
    X_val = X_val[:,:,:,:,:]
    X_val = np.transpose(X_val,[0,1,3,2])
    print ("Val Input Shape",X_val.shape)
    Y_val = Y_val[:,:,0]

    train_serial = get_srno_array(r,np.float32)
    val_serial = get_srno_array(r,np.float32)
...
def evaluate (saved_model_path, output_file_path, model,fold, name):
    name = name+"_fold_"+str(fold)
    sess = tf.Session()
    dic = get_data_generators(fold)
...
def calculate_and_store_scores (input_folder, output_folder, name):
    ...
    for fold in range(1,6):
        ...
        for strs in file_list:
            if sub_str in strs:
                ...
                strs = strs.split(".")
                strs = strs[0]
                saved = saved+os.sep+strs+'.ckpt'
                print("--"*10,saved)
                #dummy = input("wait")
                evaluate(saved,output_folder,model_graph,fold,name)
                del model_graph
...
#    calculate_and_store_scores (input_folder, output_folder, name)
